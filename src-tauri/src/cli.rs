...
use crate::{
    commands::state::{
        consts::{WindowState, WorkspaceState},
        get_state::{read_saved_state, Store},
...
fn open(handle: &AppHandle, path: &str) -> Result<(), errors::Error> {
    ...
    let local_state = serde_json::from_str::<WorkspaceState>(
        &fs::read_to_string(parsed_path.join(".iliad").join("workspace_config.json"))
    ...
    handle.manage(Mutex::new(WindowState { state }));
    ...
    if let Store::Global(global_state) = store {
        ...
        save_state("global", &serde_json::to_string(&global_state)?, &path)?;

        handle.manage(Mutex::new(global_state));
    } else {
        return Err(errors::Error::GeneralError(
            "Could not read saved global state".to_string(),
        ));
    }
