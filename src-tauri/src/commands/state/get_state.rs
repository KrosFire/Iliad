...
pub fn read_saved_state(store_type: &str, local_path: &str) -> Result<Store, Error> {
    ...
    match store_type {
        "workspace" => {
            let saved_state_string = fs::read_to_string(config_path.join("workspace_config.json"))
        ...
        "settings" => {
            let saved_state_string =
                fs::read_to_string(global_config_path.join("settings_config.json"))
                    .unwrap_or(String::new());
        ...
        _ => {
            let saved_state_string =
                fs::read_to_string(global_config_path.join("global_config.json"))
                    .unwrap_or(String::new());
