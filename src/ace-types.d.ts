// Missing types from Ace editor package
// src: https://github.com/ajaxorg/ace/blob/v1.33.1/src/tooltip.js#L15
declare module 'ace-tooltip' {
  ...
  class HoverTooltip extends Tooltip {
    ...
    onMouseMove(e, editor) {
      this.lastEvent = e
      this.lastT = Date.now()
      const isMousePressed = editor.$mouseHandler.isMousePressed
      if (this.isOpen) {
        const pos = this.lastEvent && this.lastEvent.getDocumentPosition()
        if (
          !this.range ||
    ...
    isOutsideOfText(e) {
      ...
      const line = editor.session.getLine(docPos.row)
      if (docPos.column == line.length) {
        const screenPos = editor.renderer.pixelToScreenCoordinates(e.clientX, e.clientY)
        const clippedPos = editor.session.documentToScreenPosition(docPos.row, docPos.column)
    ...
    showForRange(editor, range, domNode, startingEvent) {
      ...
      const position = renderer.textToScreenCoordinates(range.start.row, range.start.column)
      ...
      const labelHeight = element.clientHeight
      const labelWidth = element.clientWidth
      const spaceBelow = window.innerHeight - position.pageY - renderer.lineHeight
